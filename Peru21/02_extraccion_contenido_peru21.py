"""
Extraccion de contenido (Web Scraping) para noticias de Peru21.

Lee el CSV de enlaces generado por 01_recoleccion_links_peru21.py, descarga cada
articulo, limpia el ruido (firmas, tags, relacionados, embeds) y extrae el texto
principal en una sola celda ('texto_completo').

Como el listado NO trae autor / resumen / tags, este paso tambien los completa a
partir del HTML del articulo, para mantener el mismo esquema que los otros medios.
"""
...
ARCHIVO_ENTRADA = os.path.join(BASE_DIR, "peru21_politica.csv")
ARCHIVO_SALIDA = os.path.join(BASE_DIR, "peru21_noticias_completas.csv")
...
FRASES_RUIDO = [
    "TE SUGERIMOS", "TE RECOMENDAMOS", "TE PUEDE INTERESAR", "LEE TAMBIÉN",
    "LEE TAMBIEN", "LEER RESUMEN", "MÁS INFORMACIÓN", "MAS INFORMACION",
    "VIDEO RECOMENDADO", "NEWSLETTER",
]
...
def extraer_datos_noticia(url):
    ...
    try:
        ...
        # Autor: primer enlace a /autor/ con texto (normalizando dobles espacios)
        autor = ""
        ...
        # Tags: cada etiqueta es un <div class="item-tags-full"> con el nombre
        # directamente como texto (no es un enlace).
        tags = ""
        nombres_tags = []
        for bloque in soup.find_all(class_=re.compile(r"item-tags-full", re.I)):
            t = bloque.get_text(strip=True)
            if t:
                nombres_tags.append(t)
        if nombres_tags:
            tags = ", ".join(dict.fromkeys(nombres_tags))  # sin duplicados, en orden
        ...
        # --------------------------------------
        # El cuerpo real esta en <div class="field--name-body ...">. Puede haber
        # varios (el de la nota + los de notas relacionadas incrustadas), asi que
        # elegimos el que tenga mas parrafos. Esto ademas evita traer los bloques
        # de "relacionados" / "ultimas noticias" que estan fuera de ese div.
        cuerpos = soup.find_all("div", class_=re.compile(r"field--name-body"))
        contenedor = None
        if cuerpos:
            contenedor = max(cuerpos, key=lambda b: len(b.find_all("p")))

        # Fallback: si no hay field--name-body, usar el <article> principal
        if not contenedor:
            contenedor = soup.find("article", class_=re.compile(r"node--type-article"))
        if not contenedor:
        ...
        # A) Firmas del autor, fechas y bloques de tags
        for el in contenedor.find_all(
            class_=re.compile(r"bl-firmas|firma-teaser|firma|title-tags|item-tags|fecha-", re.I)
        ):
            el.decompose()

        # B) Embeds: tuits, videos, figuras, scripts, imagenes con leyenda
        for embed in contenedor.find_all(
            ["blockquote", "iframe", "script", "style", "figure", "aside"]
        ):
            embed.decompose()

        # C) Tarjetas de noticias relacionadas / recirculacion incrustadas
        for el in contenedor.find_all(
            class_=re.compile(r"related|recirc|te-sugerimos|mas-leidas|node--type-bloque", re.I)
        ):
            el.decompose()
...
print("Iniciando extraccion de texto para Peru21...")
...
for index, row in df_final.iterrows():
    ...
    # Completar metadatos solo si el listado no los traia (NaN cuenta como vacio)
